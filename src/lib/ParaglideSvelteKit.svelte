<script lang="ts">
	import { availableLanguageTags, setLanguageTag, onSetLanguageTag, sourceLanguageTag, languageTag } from '../paraglide/runtime';
	...
	setContext(
		...
		$page.params.lang
			? (availableLanguageTags as readonly string[]).includes($page.params.lang)
				? $page.params.lang
				: (() => {
						throw redirect(302, ('/' + $page.url.href.split($page.params.lang)[1]).replace('//', '/'));
				  })()
			: sourceLanguageTag
	);
	...
	if (import.meta.env.SSR === false) {
		onSetLanguageTag((newLanguageTag) => {
			...
			if (newLanguageTag === sourceLanguageTag) {
				...
				const route = window.location.href.replace(/^https?:\/\/[^\/]+\/(en)(\/|$)/, '').match(/^https?:\/\/[^\/]+(\/.*)/);
				...
				// Redirect to the same page with the new language
				window.location.href = route && route[1] ? '/' + newLanguageTag + route[1] : '/' + newLanguageTag;
